...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        # In this case input size is embedding size
        ...
        # Projections are required to combine 2 hidden states from bi-directional encoder. Without projection,
        # dimensionality of
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # See here why we need packing/unpacking
        # https://github.com/HarshTrivedi/packing-unpacking-pytorch-minimal-tutorial
        # Input size without packing is (src_len, b, e). This can't be fed directly into LSTM.
        # See also here - https://stackoverflow.com/questions/51030782/why-do-we-pack-the-sequences-in-pytorch
        ...
        enc_unpacked, _ = pad_packed_sequence(enc_hiddens)

        # Swap dimensions. Encoding hidden should be of shape (b, src_len, h*2)
        enc_hiddens = enc_unpacked.permute(1,0,2)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        #for chunk in torch.split(Y, Y.shape[0]): # Iterate over time dimension of Y
        for i in range(Y.shape[0]):
            Y_t = Y[i] # torch.squeeze(chunk, dim=0)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.squeeze(torch.bmm(torch.unsqueeze(dec_hidden,dim=1), enc_hiddens_proj.permute(0,2,1)),
                            dim=1)
